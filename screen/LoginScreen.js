...
export default connect(mapStateToProps)(function LoginScreen(props) {
  const [name, setName] = useState("")
  const [password, setPassword] = useState("")
  const [isModalVisible, setIsModalVisible] = useState(false)

  const goToHome = () => {
    const { connexion } = props.route.params
    connexion(true)
    console.log(connexion)
    // props.navigation.navigate("Home")
  }
  const goToRegister = () => {
    props.navigation.navigate("Register", { name })
  }
  const goToForgotenPassword = () => {
    props.navigation.navigate("ForgotPassword", { name })
  }
  console.log(props.connexion)
  return (
      ...
      <View style={styles.container}>
        ...
        <View style={{ marginHorizontal: 32 }}>
          ...
          <View style={{ alignItems: "flex-end", marginTop: "5%" }}>
            <TouchableOpacity onPress={goToForgotenPassword}>
              <Text style={{ color: "#514E5A" }}>Forget password?</Text>
            </TouchableOpacity>
          </View>
